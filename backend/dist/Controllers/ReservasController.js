...
class ReservaController {
    ...
    static async addReserva(req, res) {
        ...
        if (cedula) {
            ...
            if (!(await UsuariosController_1.UsuariosController.usuarioExisteByName(nombre))) {
    ...
    static async deleteReserva(req, res) {
        ...
        if (cedula) {
            ...
            if (!(await UsuariosController_1.UsuariosController.usuarioExisteByName(usuarioInput))) {
    ...
    static configurarReseteoDiario() {
        // Reseteo Domingo
        node_cron_1.default.schedule('0 13 * * 0', async () => {
            console.log('Reseteando reservas a las 13:00 (domingo)...');
            await ReservaController.resetearHorariosDiarios();
        });
        // Reseteo Lunes a Sábado
        node_cron_1.default.schedule('30 20 * * 1-6', async () => {
            console.log('Reseteando reservas a las 20:30 (lunes a sábado)...');
            await ReservaController.resetearHorariosDiarios();
        });
    }
    static async chequeoHorarioPrioritario(usuario, dia) {
        return await HorariosController_1.HorariosController.usuarioPrioritario(usuario, dia);
    }
    static calcularDiffHorarioEnMin(horaReq) {
    ...
    static esPrevioAHoraActual(tiempoStr) {
    ...
    static async calculoHorarioPrioritario(hora, usuario, res) {
        try {
            const now = new Date();
            let dia = new Intl.DateTimeFormat('es-ES', { weekday: 'long' }).format(now);
            // Si es mas de las 20:30 paso al dia siguiente
            if ((0, timeUtils_1.esMasDeLas2030)() || (0, timeUtils_1.esDomingoALas13)()) {
    ...
    static async updateReserva(req, res) {
        ...
        if (!(await UsuariosController_1.UsuariosController.usuarioExisteByName(usuario))) {
    ...
    static async getHorariosDispniblesHoy() {
        try {
            const data = await promises_1.default.readFile(ReservaController.DATA_PATH_HORARIOS, 'utf-8');
    ...
    static async getHorariosDispniblesMañana() {
        try {
            const data = await promises_1.default.readFile(ReservaController.DATA_PATH_HORARIOS, 'utf-8');
            const horarios = JSON.parse(data);
            const uniqueHours = new Set();
    ...
    static async agregarReserva(hora, usuario) {
        ReservaController.reservas[hora].push({ hora, usuario });
        await promises_1.default.writeFile(ReservaController.DATA_PATH_RESERVAS, JSON.stringify(ReservaController.reservas, null, 2));
    }
    static async borrarReserva(hora, usuario, index) {
        ReservaController.reservas[hora].splice(index, 1);
        await promises_1.default.writeFile(ReservaController.DATA_PATH_RESERVAS, JSON.stringify(ReservaController.reservas, null, 2));
    }
    static formatBackupDataToCSV(backupReservas) {
        ...
        return (0, json2csv_1.parse)(rows, { fields: timeSlots });
    }
    static async sendBackupEmail(backupReservas) {
        try {
    ...
    static async resetearHorariosDiarios() {
        ...
        const data = await promises_1.default.readFile(ReservaController.DATA_PATH_RESERVAS, 'utf-8');
        const backupReservas = JSON.parse(data);
        await this.sendBackupEmail(backupReservas);
        const horas = await this.getHorariosDispniblesMañana();
    ...
    static async inicializarHorariosDiarios() {
        ...
        if (Object.keys(backupReservas).length === 0 && backupReservas.constructor === Object) {
            ...
            if ((0, timeUtils_1.esMasDeLas2030)() || (0, timeUtils_1.esDomingoALas13)()) {
                const horas = await this.getHorariosDispniblesHoy();
                ReservaController.reservas = {};
                horas.forEach(hour => {
                    ReservaController.reservas[hour] = [];
                });
    ...
    static async inicializarHorariosDiariosNoBackup() {
        ...
        const horaActual = now.getHours();
        const minutosActuales = now.getMinutes();
        if ((0, timeUtils_1.esMasDeLas2030)()) {
            const horas = await this.getHorariosDispniblesMañana();
            ReservaController.reservas = {};
            horas.forEach(hour => {
                ReservaController.reservas[hour] = [];
    ...
    static async buscarHoraPorCedula(req, res) {
...
ReservaController.reservas = {};
ReservaController.MAX_RESERVAS_POR_HORARIO = 9;
ReservaController.DATA_PATH_HORARIOS = path_1.default.join(__dirname, '../data/HorariosPrioritarios.json');
ReservaController.DATA_PATH_RESERVAS = path_1.default.join(__dirname, '../data/BackupReservas.json');
